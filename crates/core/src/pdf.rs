...
pub fn extract_text(bytes: &[u8]) -> Result<String, ArxivError> {
    let result =
        pdf_inspector::process_pdf_mem(bytes).map_err(|e| ArxivError::ParseError(e.to_string()))?;
    result.markdown.ok_or_else(|| {
        ArxivError::NoContentAvailable("PDF produced no markdown output".to_string())
    })
}
